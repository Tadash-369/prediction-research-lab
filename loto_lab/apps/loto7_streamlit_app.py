...
def render_balance_research_details(balance_stats, diagnostics, reports=None, draw_size=7):
    ...
    render_balance_weight_research_ui(
        ...
        key_prefix="loto7_balance_weight",
        allow_review=not is_light_smoke_mode(),
        weight_research=weight_research,
        show_core_tables=False,
    )
    render_adoption_dry_run_ui(
        reports,
        game="loto7",
        review_history_path=WEIGHT_REVIEW_HISTORY_CSV,
        approval_history_path=WEIGHT_APPROVAL_HISTORY_CSV,
        key_prefix="loto7_balance_adoption",
        allow_write=not is_light_smoke_mode(),
        light_smoke=is_light_smoke_mode(),
    )
...
def render_light_smoke_overview():
    st.success("PRL_LIGHT_SMOKE=1: ロト7軽量スモークモードで起動しています。")
    ...
    with tab_status:
        latest_round = "-"
        if not results.empty and "開催回" in results.columns:
            values = pd.to_numeric(results["開催回"], errors="coerce").dropna()
...
def render_chamini6_prediction(results, target_round, ai_weight_summary, set_ball_analysis):
    ...
    with st.expander(f"{CHAMINI6_GOD_MODE_LABEL}（統合候補）", expanded=False):
        ...
        if st.button(f"{CHAMINI6_GOD_MODE_LABEL}予測を保存", key="loto7_chamini6_save"):
            result = save_prediction_picks(picks, target_round, CHAMINI6_GOD_MODE_KEY, CHAMINI6_GOD_MODE_LABEL)
            if result["saved"]:
                st.success(f"{CHAMINI6_GOD_MODE_LABEL}予測を {result['saved']} 件保存しました。")
            else:
                st.info(f"同じ{CHAMINI6_GOD_MODE_LABEL}予測は保存済みです。")
            if result["skipped"]:
...
def build_ai_weighted_number_reasons(numbers, score_df, low_threshold=18):
    rows = []
    row_map = {int(row["数字"]): row for _, row in score_df.iterrows()}
    odd_count = sum(number % 2 for number in numbers)
    low_count = sum(number <= low_threshold for number in numbers)
    for number in numbers:
        ...
        if ai_rank > 7 and odd_count in (3, 4):
            reasons.append("奇数偶数バランス調整のため採用")
        if ai_rank > 7 and low_count in (2, 3, 4):
            reasons.append("高低バランス調整のため採用")
        if not reasons:
...
def render_prediction_area(results):
    ...
    ai_weight_summary = build_ai_improvement_weight_summary(AI_IMPROVEMENT_DIR, "loto7")
    ...
    elif should_auto_resolve_fallback(model_decision, model_key):
        recovery_result = resolve_prediction_fallback(model_decision, target_round)
        if recovery_result.get("resolved_count", 0):
            st.success(
                f"runtime設定が正常に復旧し、{MODEL_LABELS[model_key]}を標準買い目生成に再採用しました。"
...
def render_purchase_manager(result_history):
    ...
    try:
        evaluated = evaluate_purchase_history(purchases, result_history, "loto7", 7, 37)
    except Exception as exc:
    ...
    game_purchases = evaluated[evaluated["lottery_type"].astype(str) == "loto7"].copy() if not evaluated.empty else evaluated
    render_purchase_summary(evaluated, "loto7")
    ...
    with st.form("loto7_purchase_form"):
        ...
        with col_right:
            ...
            cost = st.number_input("購入金額", min_value=0, value=300, step=100)
    ...
    if submitted:
        numbers, errors = validate_purchase_numbers(numbers_text, 7, 37)
        if errors:
            for error in errors:
                st.warning(error)
        else:
            ...
            try:
                ...
                updated = evaluate_purchase_history(updated, result_history, "loto7", 7, 37)
    ...
    st.markdown("**購入履歴一覧**")
    display = purchase_display_df(evaluated, "loto7")
    if display.empty:
        st.info("ロト7の購入履歴はまだありません。")
    else:
        st.dataframe(display, width="stretch", hide_index=True)
        if st.button("照合結果を purchases.csv に反映", key="loto7_purchase_sync"):
            try:
...
def render_lab():
    ...
    if fixed_predictions.empty:
        ...
        st.caption(f"状態: {fixed_status}。抽選結果登録後、この一覧を検証レポートへ照合します。")
    ...
    with tabs[0]:
        st.markdown("**新フロー**")
        st.dataframe(build_research_flow_table(), width="stretch", hide_index=True)
        if len(result_history) <= 40:
            st.info("バックテストには最低41回分以上の抽せん結果が必要です。")
        else:
            max_rounds = max(10, min(30, len(result_history) - 40))
            ...
            if st.button("ロト7バックテストを実行"):
                with st.spinner("機械学習モデルを含めてロト7モデルを検証しています..."):
                    summary_df, detail_df = run_backtest(result_history, lookback_rounds=lookback_rounds)
    ...
    with tabs[2]:
        ...
        if ranking.empty and dashboard.empty:
            ...
            with st.expander("ChaminiSP / バランス仮説 研究成績", expanded=False):
                ...
                st.dataframe(build_chamini_sp_performance_summary(reports, draw_size=7), width="stretch", hide_index=True)
                balance_stats = build_balance_hypothesis_performance(reports, draw_size=7)
                ...
                unverified = build_unverified_chamini_sp_predictions(predictions, official, reports, draw_size=7, number_max=37)
                st.markdown("**未検証のChaminiSP保存済み予想**")
                if unverified.empty:
                    st.info("未検証のChaminiSP予想はありません。")
                else:
                    st.dataframe(unverified, width="stretch", hide_index=True)
                diagnostics = build_balance_verification_diagnostics(predictions, official, reports, draw_size=7, number_max=37)
                render_balance_research_details(balance_stats, diagnostics, reports=reports, draw_size=7)
